...
//            this.driver.findElement(parseLocator(element));
        } catch (Exception var4) {
...
//            LOG.info("Going for  "+ timeInMilliSeconds + "delay" );
    ...
    public void Dropdown_Select(String element1, String element2, String value) throws Exception {
        By object1 = LocatorUtil.getLocator(element1);
        By object2 = LocatorUtil.getLocator(element2);
        ...
        while (retryCount < maxRetries) {
            try {
                WebElement input = driver.findElement(object1);
                input.click();
                ...
                String dropdownText = driver.findElement(object2).getText();

                if (dropdownText.contains(value)) {
                ...
                LOG.warn("Retrying dropdown selection due to exception: " + e.getMessage());
                retryCount++;
                Thread.sleep(1000);
            }
        }
        ...
        if (retryCount == maxRetries) {
            LOG.error("Failed to select value from dropdown after " + maxRetries + " attempts.");
        }
    }
    ...
    public static void selectCheckboxForMatchingRow(WebDriver driver, String tableLocator, String textToMatch) throws Exception {
        // Locate all rows under the given table (excluding thead)
        By rowLocator = LocatorUtil.getLocator(tableLocator + "/tr");
        List<WebElement> rows = driver.findElements(rowLocator);

        for (WebElement row : rows) {
            // Skip header rows by checking for <th>
            if (!row.findElements(By.tagName("th")).isEmpty()) {
                continue; // It's a header row, skip
            }

            List<WebElement> cells = row.findElements(By.tagName("td"));

            if (cells.size() >= 2) { // Ensure at least 2 columns exist
                String col2Text = cells.get(1).getText().trim();

                if (col2Text.equalsIgnoreCase(textToMatch.trim())) {
                    // Click checkbox in column 1
                    WebElement checkbox = cells.get(0).findElement(By.xpath(".//input[@type='checkbox']"));

                    if (!checkbox.isSelected()) {
                        checkbox.click();
                        System.out.println("✔ Checkbox selected for row with column 2 text: " + textToMatch);
                    } else {
                        System.out.println("✔ Checkbox already selected for: " + textToMatch);
                    }
        ...
        System.out.println("❌ No matching row found with column 2 text: " + textToMatch);
    }
